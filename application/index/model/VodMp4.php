 ...
 *        
 */
class VodMp4 extends Base
{

    protected $autoWriteTimestamp;

    protected $name = 'vod_mp4';
    ...
    public function saves($data)
    {
        ...
        $result = Db::name('vod_mp4')->where('id=' . $id)->update($data);
    ...
    public function getList($where, $field = '*', $page = 1, $limit = 10, $order = 'id desc')
    {
        ...
        if ($count) {
            ...
            $list = Db::name('vod_mp4')->field($field)
                ->where($where)
                ->limit($first, $limit)
                ->order($order)
                ->paginate();
